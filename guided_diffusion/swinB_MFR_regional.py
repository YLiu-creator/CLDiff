...
class SwinTransformerSys_ram(nn.Module):
    ...
    def up_x4(self, x):
        ...
        if self.final_upsample == "expand_first":
            ...
            x = x.permute(0, 3, 1, 2)  # B,4C,H,W
            pred = self.output(x)

        return x, pred

    def forward(self, x):
        # going to release after paper publication
        return cls_pre, seg_pre, cond_feat
...
class swin_RAM_Regional(nn.Module):
    def __init__(self, in_chans=4, num_classes=21843, zero_head=False):
        super(swin_RAM_Regional, self).__init__()
        self.in_chans = in_chans
        self.num_classes = num_classes
        self.zero_head = zero_head

        self.swin_ramunet = SwinTransformerSys_ram(img_size=320,
                                                   patch_size=4,
                                                   in_chans=in_chans,
                                                   num_classes=2,
                                                   window_size=10,
    ...
    def forward(self, x):
        cls_pre, seg_pre, cond_feat = self.swin_ramunet(x)
        return cls_pre, seg_pre, cond_feat
