...
import string
import random
from .swinTransformer import swin_base_patch4_window12_384

from guided_diffusion.hybridLoss import SSIM
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.backbone = swin_base_patch4_window12_384(num_classes=2)

        self.cam8x_compress = nn.Sequential(nn.Conv2d(in_channels=256, out_channels=64, kernel_size=1, stride=1),
                                            nn.BatchNorm2d(64),
                                            nn.ReLU(True),
                                            ...
                                            nn.BatchNorm2d(64),
                                            nn.ReLU(True))
        self.cam16x_compress = nn.Sequential(nn.Conv2d(in_channels=512, out_channels=64, kernel_size=1, stride=1),
                                            nn.BatchNorm2d(64),
                                            nn.ReLU(True),
                                            nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, padding=1),
                                            nn.BatchNorm2d(64),
    ...
    def p_mean_variance(
        self, model, x, en_feat4x, en_feat8x, en_feat16x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None
    ):
        """
        ...
        cal = 0
        assert t.shape == (B,)
        # during test, model is swinUNet
        # the model_output is a tuple, 0 is [b,2,h,w] and 1 is [b,1,h,w]
        model_output, cal, cls_pre = model(x, self._scale_timesteps(t), en_feat4x, en_feat8x, en_feat16x, **model_kwargs)
    ...
    def p_sample(
        ...
        return {"sample": sample, "pred_xstart": out["pred_xstart"], "cal": out["cal"], "diff_oup": out["diff_oup"]}
    ...
    def p_sample_loop_known(
        ...
        img = img.to(device)
        en_feat4x = en_feat4x.to(device)
        en_feat8x = en_feat8x.to(device)
        en_feat16x = en_feat16x.to(device)
        ...
        if self.dpm_solver:
            ...
            model_fn = model_wrapper(
                ...
                model_kwargs=model_kwargs,
                en_feat4x=en_feat4x, en_feat8x=en_feat8x, en_feat16x=en_feat16x)

            dpm_solver = DPM_Solver(model_fn, noise_schedule,
                                    algorithm_type="dpmsolver++",
                                    correcting_x0_fn="dynamic_thresholding",
                                    img = img[:, :-1,  ...],
                                    en_feat4x=en_feat4x, en_feat8x=en_feat8x, en_feat16x=en_feat16x, en_normx=en_normx)

            ## Steps in [20, 30] can generate quite good samples.
            # diff_out, bbone_seg, bbone_cls

            sample, cal,bbone_cls = dpm_solver.sample(
        ...
        return final["sample"], x_noisy, img, final["cal"], cal_out, final["diff_oup"]
    ...
    def p_sample_loop_progressive(
        ...
        org_c = img.size(1)
        org_RSI = img[:, :-1, ...]      #original remotes ensing image
        ...
        if progress:
           ...
           for i in indices:
                ...
                with th.no_grad():
                    ...
                    if img.size(1) != org_c:
                        img = torch.cat((org_RSI, img), dim=1)       #in every step, make sure to concatenate the original image to the sampled segmentation mask
    ...
    def ddim_sample_loop_progressive(
        ...
        for i in indices:
                ...
                with th.no_grad():
                 ...
                 if img.shape != (img.shape[0], 5, 320, 320):
    ...
    def training_losses_segmentation(self, model, classifier, x_start, t,
                                     HOT_sup, en_feat4x, en_feat8x, en_feat16x,epoch_index,
                                     model_kwargs=None,
                                     noise=None):
        """
        ...
        mask = x_start[:, -1:, ...] # mask is the ground truth
        res = HOT_sup
        # res = torch.where(mask > 0, 1, 0)   #merge all tumor classes into one to get a binary segmentation mask

        # generate block label
        block_tar = torch.zeros((int(mask.shape[0])))
        for index in range(int(x_start.shape[0])):
            cld_ratio = torch.sum(mask[index, ...]) / (320.0 * 320.0)
            if cld_ratio >= 0.2:
                block_tar[index] = 1
            else:
                block_tar[index] = 0
        block_tar = block_tar.to(x_start.device, dtype=th.long)
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.BCE_DICE or self.loss_type == LossType.RESCALED_MSE:
            # here the model is the swinUNet or UNetModel_newpreview
            # x_t is [1,5,320,320]
            # model_output is [1,2,320,320], cal is [1,1,320,320], cls is [1,2]
            model_output, cal, cls_pre = model(x_t, self._scale_timesteps(t), en_feat4x, en_feat8x, en_feat16x, **model_kwargs)

            # Gussian moudlation
            mean = torch.mean(cal)
            std = torch.std(cal)
            sigma = math.sqrt(2 * math.pi) * std
            cal_gus = torch.exp(-(cal - mean) ** 2 / (2 * std ** 2)) / sigma

            pure_map = torch.ones_like(cal)
            pure_map[cal >= 0.8] = 0
            pure_map[cal <= 0.2] = 0
            cal = cal + pure_map * cal_gus
            ...
            # 计算t时刻真值target和t刻预测结果model_output之间的差异, MSEloss
            # terms["loss_diff"] = mean_flat((target - model_output) ** 2 )
            terms["loss_diff"] = nn.MSELoss()(model_output, target)

            # 计算输入图像对应的ground truth和条件网络输出之间的差异, MSEloss
            # terms["loss_cal"] = mean_flat((res - cal) ** 2)
            # our defined
            # cal is [2,1,320,320]
            # terms["loss_cal"] =  nn.MSELoss()(cal, res.type(th.float32))

            alpah = 0.4
            terms["loss_mfrcls"] = nn.CrossEntropyLoss()(cls_pre, block_tar)
            terms["loss_mfrseg"] = nn.MSELoss()(cal, HOT_sup.type(th.float32))
            terms["loss_mfrssim"] = SSIM(cal, HOT_sup.type(th.float32))
            terms["loss_mfr"] = alpah * terms["loss_mfrcls"] + (1 - alpah) * (terms["loss_mfrseg"]+ terms["loss_mfrssim"])
